...
function Send() {
  ...
  const handleSend = async () => {
    ...
    if (hasInsufficientBalance()) {
      toast.error(
        `Insufficient balance. You have ${formatNumber(tokenBalance)} ${
          transaction.token
        }`
      );
      return;
    ...
    if (balancePercentage > 95) {
      const confirmed = window.confirm(
        `You are using ${balancePercentage.toFixed(1)}% of your ${
          transaction.token
        } balance. ` +
  ...
  return (
    <div className="h-full flex-col items-center justify-center w-full py-[20px]">
      ...
      <div className="flex items-center justify-between border-b border-gray-500 px-[20px] pb-[20px]">
        <button
          onClick={() => navigate(-1)}
          className="p-[5px] rounded-full z-20 border border-[#44F58E] bg-black"
        >
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-[15px]">
                <TokenLogo
                  ...
                  size={50}
                  variant={getTokenVariant(
                    transaction.token,
                    transaction.name || transaction.token
                  )}
                ...
                {isLoadingBalances ? (
                  ...
                  <>
                    <p className="text-white font-[600]">
                      Balance: {formatNumber(tokenBalance)} {transaction.token}
                    </p>
                    <p className="text-gray-400 text-sm">
                      ≈ ${(tokenBalance * tokenPrice).toFixed(2)}
                    </p>
                  </>
                )}
              </div>
          ...
          <div className="border border-gray-600 rounded-[15px] p-[20px]">
            ...
            <div className="mt-[10px] flex items-center justify-between">
              <span className="text-gray-400 text-sm">
                ≈ ${isNaN(usdValue) ? "0.00" : usdValue.toFixed(2)} USD
              </span>
              {amount && parseFloat(amount) > 0 && (
                <span className="text-gray-500 text-xs">
                  Remaining: {formatNumber(getRemainingBalance())}{" "}
                  {transaction.token}
                </span>
              )}
            </div>
            ...
            {hasInsufficientBalance() && (
              <div className="flex items-center gap-2 mt-3 p-2 bg-red-900/20 border border-red-500/30 rounded text-red-400 text-xs">
                <AlertCircle size={14} />
                <span>
                  Insufficient {transaction.token} balance. You have{" "}
                  {formatNumber(tokenBalance)} {transaction.token}
                </span>
              </div>
            )}
          </div>
            ...
            <div className="bg-[#0a0a0a] border border-[#44F58E]/30 rounded-[15px] p-[20px]">
              ...
              <div className="space-y-[10px]">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-[#44F58E] font-[600]">
                    {parseFloat(amount) + 0.001} {transaction.token}
                  </span>
                </div>
              </div>
            </div>
